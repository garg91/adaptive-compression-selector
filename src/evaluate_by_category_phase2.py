...
OUTPUT_JSON_PATH = Path("results/category_evaluation_phase2.json")
REPORT_PATH = Path("results/category_report_phase2.txt")
...
def evaluate_selector_by_category(by_file, mode):
    ...
    for _, compressor_rows in by_file.items():
        ...
        if chosen not in compressor_rows:
            # Phase 2 may skip a tiny number of compressor/file combinations.
            # If the selector chose a skipped compressor, fall back to the
            # best available compressor by compressed size so the category
            # report still covers the file instead of silently dropping it.
            fallback = min(
                compressor_rows.values(),
                key=lambda row: row["compressed_size_bytes"],
            )
            chosen = fallback["compressor"]
            row = fallback
        else:
        ...
        add_row(category_metrics[key], row, chosen=chosen)
