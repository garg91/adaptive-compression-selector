...
SELECTOR_EVAL_PATH = Path("results/selector_evaluation_phase2.json")
PARETO_EVAL_PATH = Path("results/pareto_selector_evaluation_phase2.json")

REPORT_PATH = Path("results/final_summary_report_phase2.txt")
REPORT_JSON_PATH = Path("results/final_summary_report_phase2.json")
...
def benchmark_category_stats():
    raw_rows = load_json(BENCHMARK_PATH)
    rows = [
        r for r in raw_rows
        if "error" not in r
        and not r.get("skipped", False)
        and "original_size_bytes" in r
    ]
...
def build_final_takeaways(selector_by_name):
    ...
    lines.append(
        "5. gzip_9 and zlib_9 should remain as baselines unless compatibility is required; "
        "the Pareto/category reports should be used to confirm where they are dominated."
    )
